...
keyurl={
    "womail_url": str(os.environ['WOMAIL_URL_LIST'])
}
...
class WoMail():
    ...
    def dotask(cookies):
        ...
        headers = {
            "User-Agent": "User-Agent: Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.25 Safari/537.36 Core/1.70.3868.400 QQBrowser/10.8.4394.400",
            "Cookie": cookies,import requests
        ...
        try:
            ...
            if dated < end_time:
                ...
                if res["success"]:
                    ...
                    msg.append({"name": "抽奖结果", "value": f"获取拼图个数失败"})
            else:
                msg.append({"name": "抽奖结果", "value": f"活动已结束，不再执行"})
        except Exception as e:
            msg.append({"name": "抽奖结果", "value": str(e)})
        return msg

    def main(self):
        url = self.check_item.get("womail_url")
        try:
...
def main_handler(event, context):
    print(WoMail(check_item=keyurl).main())
